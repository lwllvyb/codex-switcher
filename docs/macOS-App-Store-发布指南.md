...
- 仓库里不再保存真实 `DEVELOPMENT_TEAM`，拉代码后要在你本地补上。
- 仓库里的 `PRODUCT_BUNDLE_IDENTIFIER` 现在是公开示例值 `com.example.codex.switcher`。
- `CODE_SIGN_ENTITLEMENTS` 已经指向 `multi-codex-limit-viewer/MultiCodexLimitViewer.entitlements`。
- 如果你要上 Mac App Store，还需要把 `App Sandbox` 打开，并按实际能力收紧 entitlements。
...
如果 `com.example.codex.switcher` 只是仓库里的示例值，先换成你自己的正式 Bundle ID，再去 App Store Connect 创建记录。
